...
class MoeveForward(nn.Module):
    def __init__(self, num_experts, input_size, xprt_size, k, dropout,noise_scale=1.0):
        super().__init__()
    ...
    def forward(self, input, use_noisy_routing=False):
        ...
        # Add minimum bound to prevent extremely small values that can cause numerical instability
        router_noise = torch.clamp(self.softplus(self.noise_network(input)), min=1e-6)
        ...
        if self.training or use_noisy_routing:
           ...
           routing_logits_noisy = router_logits + torch.randn_like(router_logits) * (router_noise * self.noise_scale)
        ...
        for expert_idx, expert_network in enumerate(self.experts):
            ...
            # Add epsilon to prevent division by very small numbers that can cause NaN
            eps = 1e-8
            noise_denom = torch.clamp(router_noise.transpose(0, 1)[expert_idx], min=eps)
            expert_selection_prob = normal.cdf((router_logits_expert - kth_best_logits) / noise_denom)
...
class Encoder(nn.Module):
    def __init__(self, hidden_size, heads, num_experts, expert_hidden_size, k, dropout, noise_scale=1.0):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, input_size, hidden_size, encoding_size, g_norm, heads, num_xprtz, xprt_size, k, dropout_encoder, layers, output_size, noise_scale=1.0):
        super().__init__()
